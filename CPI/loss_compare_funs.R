source("~/Downloads/permimp 2/R/Utils.R")
                      ...
                      w, AUC, pre1.0_0, mincriterion, asParty)
{
  ...
  for (treeNr in seq_len(ntree)){
    ...
    poob <- pred(tree, inp, mincriterion, -1L, input)
    eoob <- error(poob, oob, y)
    ...
    for(j in varsInTree){
        ...
        ## p should now be the permuted predictions, while poob would be 
        ## the unpermuted oob predictions. 
        ## And y shall be the model response. 
        curr_y = y[oob]
        
        loss_1 = mean((newy- poob)^2)
        loss_2 = mean((newy - p)^2)
        loss_3 = mean((curr_y-p)^2)
...
samples_perm <- function(y, varID, varsToCondOn, binnedVars, oob, asParty) 
{
