...
linear_truth = copula_ff  
truth_vec = rep(linear_truth, each = 3)
truth_mat = matrix(rep(truth_vec, each =200), nrow =27, byrow = T)


linear_data = read.csv("~/Working/Ning/Giles_Project_1/Code/LOCO CPI tMLE/2024-04-12/full_truth_linear_var_copula_1000.csv")[, -1]
...
long_data <- melt(coverage, varnames = c("Method", "Rho"), value.name = "Coverage")
long_data$Sequence <- as.numeric(as.character(long_data$Sequence))

# Plotting with ggplot2
ggplot(long_data, aes(x = Rho, y = Coverage, group = Method, color = Method)) +
  geom_line() +
  geom_point() +
  labs(title = "Coverage of Truth rf with copula", x = "Sequence", y = "Coverage") +
  theme_minimal() + ylim(0,1) + geom_hline(yintercept =  .95, color = "red")

ggsave("./copula_truth_coverage.png", width = 1200, height = 936, units = "px", bg ="white")
