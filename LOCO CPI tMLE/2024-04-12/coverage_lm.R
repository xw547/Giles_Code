...
linear_data = read.csv("~/Working/Ning/Giles_Project_1/Code/LOCO CPI tMLE/2024-04-12/full_lm_linear_var_1000.csv")[, -1]
...
ggplot(long_data, aes(x = Sequence, y = Coverage, group = Method, color = Method)) +
  geom_line() +
  geom_point() +
  labs(title = "Coverage of Linear Model", x = "Sequence", y = "Coverage") +
  theme_minimal() + ylim(0,1) + geom_hline(yintercept =  .95, color = "red")


ggsave("./mvnorm_lm_coverage.png", width = 1200, height = 940, units = "px", bg ="white")
