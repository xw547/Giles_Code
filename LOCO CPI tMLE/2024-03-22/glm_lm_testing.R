...
full_model = glm(output~., data = full_data[1:n_1,],  family = "gaussian")
reduced_model = glm(data= reduced_data[1:n_1,], output~., family = "gaussian")
...
### Now we fit the models.
full_model_trunc = glm(output~., data = full_data_trunc[1:n_1,],  family = "gaussian")
full_model_trunc_est = glm(output~., data = full_data_trunc[1:n_1,],  family = "gaussian")
reduced_model_trunc = glm(output~., data= reduced_data_trunc[1:n_1,], family = "gaussian")
...
first_term <-
  function(index, full_data, full_model, reduced_model) {
    mean(2*(full_model$fitted.values[index]
            - reduced_model$fitted.values[index])*(full_data[index, 1] 
