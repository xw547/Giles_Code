...
full_model = gam(output~., data = full_data[1:100,],  family = "gaussian")
reduced_model = gam(data= reduced_data[1:100,], output~., family = "gaussian")


### We can now try to compare the naive estimator and the tMLE one-step one.
naive_LOCO = -mean((full_data[101:150, 1]- predict(full_model, full_data[101:150, -1]))^2 
                   - (full_data[101:150, 1]- predict(reduced_model, reduced_data[101:150, -1]))^2)
OneStep_LOCO = mean((2*full_data[101:150, 1] 
                     - predict(full_model, full_data[101:150, -1]) 
                     - predict(reduced_model, reduced_data[101:150, -1]))*
                      (predict(full_model, full_data[101:150, -1]) -
                         predict(reduced_model, reduced_data[101:150, -1])))
...
full_data_trunc = data.frame(y = Y_trunc, X = X[1:100,])
colnames(full_data_trunc) <- c("output", 1:10)
reduced_data_trunc = data.frame(y = Y_trunc, X_reduced = X_reduced[1:100,])
colnames(reduced_data_trunc) <- c("output", 2:10)

### Now we fit the models.
full_model_trunc = glm(output~., data = full_data_trunc[1:100,],  family = "gaussian")
reduced_model_trunc = glm( output~., data= reduced_data_trunc[1:100,], family = "gaussian")
...
mean((2*full_data[1:100, 1][trunc_index]
