...
estimates_linear = abs(matrix(rowMeans(linear_data[sort(c(2+4*(0:8), 3+4*(0:8))),]), 
                              byrow = F, nrow = 2) - matrix(rep(linear,2), byrow = T, nrow= 2))
colnames(estimates_linear) <- paste0(seq(.1, .9, .1))
rownames(estimates_linear) <- c("tMLE", "Plug_in")

estimates_nonlinear = abs(matrix(rowMeans(nonlinear_data[sort(c(2+4*(0:8), 3+4*(0:8))),]), 
                                byrow = F, nrow = 2) - matrix(rep(nonlinear,2), byrow = T, nrow= 2))
colnames(estimates_nonlinear) <- paste0(seq(.1, .9, .1))
rownames(estimates_nonlinear) <- c("tMLE", "Plug_in")
