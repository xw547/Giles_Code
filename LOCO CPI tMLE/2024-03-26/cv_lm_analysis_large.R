...
nonlinear_data = read.csv("~/Working/Ning/Giles_Project_1/Code/LOCO CPI tMLE/03-26/cv_lm_nonlinear_5000.csv")[, -1]
linear_data = read.csv("~/Working/Ning/Giles_Project_1/Code/LOCO CPI tMLE/03-26/cv_lm_linear_5000.csv")[, -1]




estimates_linear = abs(rbind(matrix(rowMeans(linear_data), byrow = F, nrow = 5))
...
estimates_nonlinear =abs(rbind(matrix(rowMeans(nonlinear_data), byrow = F, nrow = 5))
...
ggplot(model_bias_long, aes(x = Rho, y = Bias, color = Model, group = Model)) +
  ...
  labs(title = "Bias of tMLE vs Plugin under linear true model",
       x = "Rho",
       y = "Bias")  +ylim(0,2)
ggsave("./cv_lm_linear_large.png", width = 1200, height = 936, units = "px", bg ="white")
...
ggsave("./cv_lm_nonlinear_large.png", width = 1200, height = 936, units = "px", bg ="white")
