...
nonlinear_data = read.csv("~/Working/Ning/Giles_Project_1/Code/LOCO CPI tMLE/03-26/cv_gam_nonlinear_1000.csv")[, -1]
linear_data = read.csv("~/Working/Ning/Giles_Project_1/Code/LOCO CPI tMLE/03-26/cv_gam_linear_1000.csv")[, -1]
naive_cv_linear = read.csv("~/Working/Ning/Giles_Project_1/Code/LOCO CPI tMLE/03-26/naive_para_cv_gam_lm_1000.csv")[,-1]
naive_cv_nonlinear = read.csv("~/Working/Ning/Giles_Project_1/Code/LOCO CPI tMLE/03-26/naive_para_cv_gam_nonlinear_1000.csv")[,-1]
...
ggplot(model_bias_long, aes(x = Rho, y = Bias, color = Model, group = Model)) +
  ...
  labs(title = "Bias of tMLE vs Plugin under linear true model",
       x = "Rho",
       y = "Bias")  +ylim(0,2)
ggsave("./cv_gam_linear.png", width = 1200, height = 936, units = "px", bg ="white")
...
ggsave("./cv_gam_nonlinear.png", width = 1200, height = 936, units = "px", bg ="white")
