...
sim_100_noforest = foreach(s = 1:160, .combine = "cbind")%dopar%{
  n = length(Y)
  N = 100
...
sim_100_noforest_withmse = rbind(sim_100_noforest, rep(mean(full_model$mse), 160))

boxplot(t(sim_100_noforest_withmse), names = c("p(y| z)", "p(x', y|z)",
                                               "p(x|z)", "model MSE"),
        main = "Same Random Forest", 
        col=c("skyblue", "lightgreen", "lightpink", "yellow"))
boxplot(t(sim_100_withforest), names = c("p(y| z)", "p(x', y|z)",
                                         "p(x|z)"),
        main = "Different Random Forest in each iteration",
