...
second_term_generate <- function(input_w, n = 100, pw, pz, full_model){
  samples = sample(pz$x, replace = T, size = n, prob = pz$y)
  predict_data_full = data.frame(w = rep(input_w, n), z = samples)
  
  full_predictions = predict(full_model, predict_data_full)
  reduced_predictions = mean(full_predictions)
  
  return(mean((full_predictions - reduced_predictions)^2))
}

second_term_tangent <- function(input_w, n = 100, pz, epsilon_model, reduced_model){
  samples = sample(pz$x, replace = T, size = n, prob = pz$y)
  interim_full = data.frame(w = rep(input_w, n), z = samples)
  predict_data_full = data.frame(curr_offset = predict(full_model, interim_full), )
  predict_data_reduced = data.frame(z = samples)
  
  full_predictions = predict(epsilon_model, predict_data_full)
  reduced_predictions = predict(reduced_model, predict_data_reduced)
  
  return(mean((full_predictions - reduced_predictions)^2))
}

second_update <- function(index, input_z, n, pw, full_model, reduced_model,
                          update_full_model){
  samples = sample(pz$x, replace = T, size = n, prob = pz$y)
  predict_data_full = data.frame(w = rep(input_w, n), z = samples)
  predict_data_reduced = data.frame(z = samples)
...
third_term_wz <- function(index, input_w, input_z, input_y, pw_index, pz_index, 
                          pwz, full_model, reduced_model, epsilon_model) {
  
  density = density_ratio(index, pw_index, pz_index, pwz)
  full_prediction = full_model$fitted.values[index]
  reduced_prediction = reduced_model$fitted.values[index]
...
third_h <- function(index, input_w, input_z, pw_index, pwcz, full_model, reduced_model){
  density_rat <- pw_index$estimate[index]/pwcz$condens[index]
  ...
  return(density_rat * (full_prediction - reduced_prediction))
}

third_update <- function(index, input_w, input_z, input_y,
                         pw_index, pwcz, full_model, reduced_model,
                         update_full_model){
  density_rat <- pw_index$estimate[index]/pwcz$condens[index]
  full_prediction <- update_full_model$fitted.values[index]
  reduced_prediction <- reduced_model$fitted.values[index]
  
  result = density_rat * (full_prediction - reduced_prediction) * (input_y - full_prediction)
...
### Calculate the original likelihood. Notice that these two are designed in the 
### hope that we can consider exponential path.
likelihood_first <- function(epsilon, y, pycwz, eif_first){
  likelihood_individual <- numeric(length(y))
  normalizing <- mean(exp(epsilon*eif_first))
  
  for (index in 1:length(y)) {
    likelihood_individual[index] <- log(pycwz$condens[index] * exp(epsilon * eif_first[index])) 
  }
  
  return(mean(likelihood_individual)/normalizing)
}
