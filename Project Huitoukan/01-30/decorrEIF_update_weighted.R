### In this document, I'll try to conduct update, since it seems that the 
### the performance is indeed better

source("~/Working/Ning/Giles_Project_1/Code/Project Huitoukan/01-30/decorrEIF_setup_updated.R")
...
### which used to be the speed constraint in the EIF estimation.
...
pw_index = kde(w, hpi(w), eval.points = w, density = T)
pz_index = kde(z, hpi(z), eval.points = z, density = T)
pwz = kde(cbind(w,z), Hpi(cbind(w,z)), eval.points = cbind(w,z), density = T)
...
while(eps > tol & counter <50){
  psi_now = psi_hat(n = 100, pw_index, pz_index, full_model, reduced_model)
  ...
  epsilon = optimise(likelihood_ori, c(-1/2, 1/2))
...
eif_vec = numeric(length(y))
decorr_vec = numeric(length(y))
...
counter = 1 
gma = 1
while(gma > tol & counter <50){
  psi_curr_1 <- hat_psi(w, z, y, pw_index, pw_index, 
                        pz_index, pz_index, pwz, full_model, reduced_model)
  psi_curr_2_1 <- hat_psi(w, z, y, pw_index_2, pw_index_2_1, 
                          pz_index_2, pz_index_2_1, pwz_2_1, full_model_2, reduced_model_2)
  ...
  gamma = optimise(likelihood_nor, c(-1/2, 1/2))
...
eif_vec_ori = numeric(length(y))
decorr_vec_ori = numeric(length(y))
