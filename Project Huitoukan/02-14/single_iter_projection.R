...
for (index in 1:length(y)){
  eif_first[index] = third_term_wz(index, w[index], z[index], y[index], 
                                   pw_index, pz_index, pwz, full_model, reduced_model)
}

epsilon_first = optimise(likelihood_first, c(-1/2, 1/2), y = y, pycwz = pycwz, eif_first = eif_first)
