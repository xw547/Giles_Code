...
source("~/Working/Ning/Giles_Project_1/Code/Project Huitoukan/02-20/decorrEIF_setup_updated.R")
  ...
  pw_index$estimate = dnorm(pw_index$eval.points, 3, 1)
  pz_index = kde(z, eval.points = z, density = T)
  pz_index$estimate = dnorm(pz_index$eval.points, 1, 1.09)
  pwz = kde(cbind(w,z), eval.points = cbind(w,z), density = T)
  sigma_matrix <- matrix(c(1, 0.3, 0.3, 1.09), ncol = 2)
  mean_vector <- c(3, 0.9)
  pwz$estimate = dmvnorm(pwz$eval.points, mean_vector, sigma_matrix)
